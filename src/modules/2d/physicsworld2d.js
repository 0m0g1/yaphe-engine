...
class PhysicsWorld2D {
    constructor(boundsWidth, boundsHeight, gravityY = 0.1, quadCapacity = 4) {
        ...
        // FIX 3: one reusable Bounds2D for all quadtree range queries.
        // Mutated in _updateParticles() — never shared across async boundaries
        // so mutation is safe.
        this._queryRange = new Bounds2D(0, 0, 0, 0);
    }
    ...
    _updateSprings()      { for (const s of this.springs)     s.update(); }
    _updateConstraints()  { for (const c of this.constraints) c.update(); }
 
    _updateParticles() {
        for (const p of this.particles) {
            if (p.fixed || p.isHeldByMouse) continue;
 
            p.applyForce(this.gravity);
            this._handleBoundary(p);
 
            // FIX 3: mutate the pre-allocated range instead of `new Bounds2D(...)`
            const r2 = p.radius * 2;
            this._queryRange.x      = p.position.x - r2;
            this._queryRange.y      = p.position.y - r2;
            this._queryRange.width  = p.radius * 4;
            this._queryRange.height = p.radius * 4;
 
            const nearby = this.quadtree.queryRange(this._queryRange);
    ...
    _handleBoundary(p) {
        ...
        if (p.position.x - r < 0)                    { p.position.x = r;                        p.deflect('x'); }
        if (p.position.x + r > this.bounds.width)    { p.position.x = this.bounds.width - r;    p.deflect('x'); }
        if (p.position.y - r < 0)                    { p.position.y = r;                        p.deflect('y'); }
